...
// totaal
  ...
  useEffect(() => {
    const newTotal = vegetableData.reduce((acc, vegetable) => acc + vegetable.countptag, 0);
    setTotal(newTotal);
  }, [vegetableData]);
  ...
  const addToCart = (item) => {
    ...
    const existingItemIndex = updatedCartItems.findIndex((cartItem) => cartItem.name === item.name);
...
<div className='container'>
        ...
        <h1 className='h1tag' onClick={openset}>ontheshelf</h1>
        </div>
        ...
        <img src='123.png' alt='img' onClick={opencart} className='image185'></img>
        ...
        {/* 1 */}
          ...
          {hide &&
           ...
           <div className="display" key={vegetable.id} > 
                ...
                <p>{vegetable.name} - (per - {vegetable.price})</p>
                <p className="empliment">Quantity - ({vegetableQuantities[vegetable.id]})</p>
                <p>Total Price - {vegetable.price * vegetableQuantities[vegetable.id]}</p>
           <div className="flexnu">
                <p className="ptag1" onClick={() => handleIncrement(vegetable.id)}> + </p>
                <p className="ptag2" onClick={() => handleDecrement(vegetable.id)}> - </p>
           </div>
                <button className="addtocart" onClick={() => addToCart(vegetable)}>Add to Cart </button>
         </div>
         ))}
